...
int tradfri_get_lamp(char* lamp_endpoint, char *response)
{
	int len;
	assert(COAP_init() == 0);
	COAP_set_psk_key(PSK_KEY, strlen(PSK_KEY));
	COAP_set_psk_identity(PSK_IDENTITY, strlen(PSK_IDENTITY));
	COAP_set_server_addr(SERVER_IP, strlen(SERVER_IP));
	return COAP_send_get(lamp_endpoint, strlen(lamp_endpoint), NULL);
	COAP_free();
	return len;
}
...
int tradfri_turn_on_lamp(char* lamp_endpoint, char *response)
{
	...
	COAP_set_psk_identity(PSK_IDENTITY, strlen(PSK_IDENTITY));
	COAP_set_server_addr(SERVER_IP, strlen(SERVER_IP));
    len = COAP_send_put(lamp_endpoint, strlen(lamp_endpoint), TURN_ON_LAMP_PAYLOAD, strlen(TURN_ON_LAMP_PAYLOAD), response);
    COAP_free();
    return len;
}
int tradfri_turn_off_lamp(char* lamp_endpoint, char *response)
{
	...
	COAP_set_psk_identity(PSK_IDENTITY, strlen(PSK_IDENTITY));
	COAP_set_server_addr(SERVER_IP, strlen(SERVER_IP));
	len = COAP_send_put(lamp_endpoint, strlen(lamp_endpoint), TURN_OFF_LAMP_PAYLOAD, strlen(TURN_OFF_LAMP_PAYLOAD), response);
	COAP_free();
	return len;
}